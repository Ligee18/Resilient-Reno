...
# 7. Expanded Everyday Language Retrofit Database (10 Options Each)
RETROFIT_DB = {
    "flood": [
...
with output_container:
    if selected_location and selected_location != "-- Select City & Postal Code --":
        ...
        # Dynamic Priority Logic: Filter options for High and Medium risks
        actions = []
        for hazard in ["flood", "wildfire", "wind"]:
            if data[hazard] in ["High", "Medium"]:
                actions.extend(RETROFIT_DB[hazard])
                
        # Output clean card layout for dynamically gathered recommendations
        for rec in actions:
